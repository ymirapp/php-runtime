...
class AcornSqsHandlerTest extends TestCase
{
    ...
    {
        $handler = new AcornSqsHandler($this->getLoggerMock(), '/tmp');

        $this->assertFalse($handler->canHandle($this->getInvocationEventInterfaceMock()));
    }

    public function testCanHandleReturnsTrueForSqsEvent(): void
    {
        $handler = new AcornSqsHandler($this->getLoggerMock(), '/tmp');
    ...
    public function testHandleCollectsFailuresIfProcessFails(): void
    {
        ...
        $logger->expects($this->once())
               ->method('error')
               ->with($this->stringContains('Processing SQS message [id1] failed: Acorn queue job failed'));

        $handler = new AcornSqsHandler($logger, '/tmp');
    ...
    public function testHandleThrowsExceptionForWrongEvent(): void
    {
        ...
        $this->expectExceptionMessageMatches('/AcornSqsHandler cannot handle Mock_InvocationEventInterface[^\s]* event/');

        $handler = new AcornSqsHandler($this->getLoggerMock(), $this->tempDir);
